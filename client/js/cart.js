...
const displayCart = () =>{
    ...
    if (cart.length > 0) {
        ...
        //modal footer
        const total = cart.reduce((acc, el) => acc + el.price * el.quanty, 0); // acc=acumulador  el= elementos
        ...
        //mp 
        const mp = new MercadoPago("",{     //PULIC KEY
            locale: "es-AR",
        });
        ...
        const createCheckoutButton = (preferenceId) =>{
            const blicksBuilder = mp.bricks();

            const renderComponent = async () =>{
                if (window.checkoutButton) window.checkoutButton.unmount(); 
                
                await blicksBuilder.create("wallet", "wallet_container", {
                    initialization: {
